'use strict'; // Game board and hexagons
...
class GameBoard {
  ...
  isValidMove(move, player_ID) {
    ...
    var ending_pos = move[1]; // check if starting pos has player's piece
    ...
    var sec_hexagon = this.getHexagon(ending_pos); // console.log(sec_hexagon.is_empty_tile)
    //!!!!!!!!!!!!!!!!!!!!!
  ...
  performAction(unit, target) {
    if (unit.getNmae() === 'delete') {
      var target_unit = this.hexagon_list.get(ID_to_key(target)).getUnit(); // set unit as activated

      unit.performAction(); // defeat target unit

      target_unit.defeat(); // clean-up gameboard
...
class Hexagon {
  constructor(ID) {
    ...
    this.neighbour_ID = get_hexagon_neighbour_ID(ID); // console.log(ID);
  ...
  setUnit(unit) {
    ...
    this.unit = unit; // updata tile status
...
module.exports.GameBoard = GameBoard; // module.exports. GameBoard.get_hexagon_neighbour_ID =  get_hexagon_neighbour_ID;